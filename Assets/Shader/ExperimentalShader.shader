Shader "Custom/ExperimentalShader"
{
    Properties
    {
        ...
        _AnimationSpeed ("AnimationSpeed", Range(0, 5)) = 1.0
    }
    SubShader
    {
        ...
        void vert(inout appdata_full data){
            /**
            float offsetX = tex2Dlod(_PerlinNoise, float4(data.vertex.y / _OffsetSlider  / _TimeSlider, 0, 0, 0)).r / _AmplitudeSlider;
            float offsetY = tex2Dlod(_PerlinNoise, float4(0, data.vertex.z / _OffsetSlider  / _TimeSlider, 0, 0)).r / _AmplitudeSlider;
            float offsetZ = tex2Dlod(_PerlinNoise, float4(0, 0, data.vertex.x / _OffsetSlider  / _TimeSlider, 0)).r / _AmplitudeSlider;

            data.vertex.x *= offsetX;
            //data.vertex.y *= offsetY;
            data.vertex.z *= offsetX;
            */
